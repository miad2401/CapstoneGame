...
public partial class RegionMapHandler : TileMap
{
	...
	public override void _UnhandledInput(InputEvent mevent)
	{
		...
		if (mevent.GetType() == typeof(InputEventMouseButton) && mevent.IsPressed())
		{
            ...
            {
                GD.Print("Mouse Drag Started.");
                //get tilemap current position
                mapLoc = GetGlobalTransform();
                //get mouse current position
                mousePos = mouseEvent.GlobalPosition;
                mousePressed = true;
            }*/
		} /*else if (mevent.GetType() == typeof(InputEventMouseButton) && mevent.IsReleased())
        {
            GD.Print("Mouse Drag Stopped.");
            mousePressed = false;
        } else if (mevent.GetType() == typeof(InputEventMouseMotion))
        {
            InputEventMouseMotion mouseEvent = (InputEventMouseMotion)mevent;
            if (mousePressed)
            {
